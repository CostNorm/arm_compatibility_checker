# --- START OF MODIFIED FILE src/slack_bot/slack_utils.py ---
...
def format_analysis_results_blocks(
    ...
    suggestions = []
    repo_display = f"`{github_url}`"  # Use the full URL for clarity
...
def format_error_blocks(github_url: str, error: Any) -> List[Dict[str, Any]]:
    ...
    repo_display = f"`{github_url}`" if github_url else "the requested repository"
...
# --- END OF MODIFIED FILE src/slack_bot/slack_utils.py ---
